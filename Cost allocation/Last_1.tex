...
\subsection*{Long-Term Forecasting}
 \noindent$\alpha(\alpha\in(0,1))$: Quantile\\
 $L(q_\alpha,y)$: Loss function\\
 $y$: actual load\\
 $x^(i)$: Gibbs sampling variable for stage $i$\\
 $N(\rho,\mu,\sigma)$: Gaussian distribution\\
 $\rho$:\\
 $\mu_X$: mean for parameter $X$\\
 $\sigma_X$: standard deviation for parameter $X$\\
 $F$: cumulative distribution function (CDF)\\
 $\beta$: coefficient of linear regression\\
 $\textbf{X}$: training data or vector\\
 $\hat{F}(X)$: estimation for real function\\
 $b_m$:\\
 $g_m(\textbf(x)_i)$: gradient of loss function\\
 $textbf{a}_m$: output for optimization\\
 $K$: the number of samples\\
 $p(x)$: probability density of $x$\\
 $P(x)$: marginal probability of $x$\\
 \\
 \subsection*{Stochastic Cooperative Games}
 \noindent$\Gamma(\mathbb{N},\Phi _{{\mathcal{F}_S}}^\alpha, {\vec \Delta _i}$: stochastic cooperative game\\
 $\mathbb{N}$: the number of player in game theory\\
 $\mathbb{S}$: coalition;subset of $\mathbb{N}$\\
 $F_S$: probability distribution of coalition $\mathbb{S}$\\
 $\mathcal{F}_S$: continuous distribution using weak convergence\\
 $\Phi _{{\mathcal{F}_S}}^\alpha$: utility function for $\mathbb{S}$ in $\alpha$\\
 ${\vec \Delta _i}$: preference\\
 $X,Y$: random variable\\
 $\psi(X,Y)$: standard distance in phase space\\
