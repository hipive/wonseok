...
\renewcommand\nomgroup[1]{%
\item[\bfseries
\ifstrequal{#1}{L}{Long-Term Forecasting}{%
\ifstrequal{#1}{S}{Stochastic Cooperative Games}{}}%
]}
...
% \IEEEPARstart{T}{he}
\nomenclature[L]{$\alpha$}{Quantile}%
\nomenclature[L]{$L(q_\alpha,y)$}{Loss function}%
\nomenclature[L]{$y$}{Actual load}%
\nomenclature[L]{$x^(i)$}{Gibbs sampling variable for stage $i$}%
\nomenclature[L]{$N(\rho,\mu,\sigma)$}{Gaussian distribution}%
\nomenclature[L]{$\rho$}{Distribution}%
\nomenclature[L]{$\mu_X$}{Mean for parameter $X$}%
\nomenclature[L]{$\sigma_X$}{Standard deviation for parameter $X$}%
\nomenclature[L]{$F$}{Cumulative distribution function (CDF)}%
\nomenclature[L]{$\beta$}{Coefficient of linear regression}%
\nomenclature[L]{$\textbf{X}$}{Training data or vector}%
\nomenclature[L]{$\hat{F}(X)$}{Estimation for real function}%
\nomenclature[L]{$b_m$}{What is this}%
\nomenclature[L]{$g_m(\textbf(x)_i)$}{Gradient of loss function}%
\nomenclature[L]{$textbf{a}_m$}{Output for optimization}%
\nomenclature[L]{$K$}{The number of samples}%
\nomenclature[L]{$p(x)$}{Probability density of $x$}%
\nomenclature[L]{$P(x)$}{Marginal probability of $x$}

\nomenclature[S]{$\Gamma(\mathbb{N},\Phi _{{\mathcal{F}_S}}^\alpha, {\vec \Delta _i})$}{Stochastic cooperative game}%
\nomenclature[S]{$\mathbb{N}$}{The number of player in game theory}%
\nomenclature[S]{$\mathbb{S}$}{Coalition;subset of $\mathbb{N}$}%
\nomenclature[S]{$F_S$}{Probability distribution of coalition $\mathbb{S}$}%
\nomenclature[S]{$\mathcal{F}_S$}{Continuous distribution using weak convergence}%
\nomenclature[S]{$\Phi _{{\mathcal{F}_S}}^\alpha$}{Utility function for $\mathbb{S}$ in $\alpha$}%
\nomenclature[S]{${\vec \Delta _i}$}{Preference}%
\nomenclature[S]{$X,Y$}{Random variable for payoff}%
\nomenclature[S]{$\psi(X,Y)$}{Standard distance in phase space}%
...
%% Why do we have to expand the distribution??
\textcolor{red}{Grid-modernization (GM) based on decarbonization is activated, and the penetration ratio of a renewable energy resource (RES) into distribution is increasing. In distribution, RES enables consumers to produce surplus resources that exceed their demand~\cite{kotler2010prosumer}. Consequently, consumers have become compatible entities capable of selling surplus energy, rather than a passive subject that only consume electricity. However, since conventionally constructed distribution networks have uni-directional characteristics and are not suitable for embracing new markets for GM~\cite{haffner2008multistage}. 
In addition, the appearance of energy storage has led to the concept of electricity transport without physical constraints, and demand shifting in various forms not meshed with criteria demand is performed inside distribution~\cite{bradbury2014economic}. For example, arbitrage for charging electricity at a period low retail price and discharging at a high price can be realized, and the market can be formed by using a storage system in the vehicle to grid (V2G). Regardless of the market, all forms of energy transactions using energy storage use distribution, but existing networks were installed without reflecting these markets. Therefore, to fully accept GM, new network expansion is required at the distribution step.}

%% How did they allocate costs before??
\textcolor{red}{Once distribution network expansion is planned, network operators, generators, load service entities (LSEs), and all other utilities related to the expansion line must participate in the cost allocation. A typical cost allocation method is the postage-stamp~\cite{kirschen1997contributions}. The postage-stamp determines the cost allocation ratio by calculating the flow of the expansion line and the percentage of generation or demand derived from the line. Another method is to determine the utilities according to the capacity, region, and performance of the line in advance~\cite{fink2011survey}. Since such a system has an exclusive characteristic for line expansion, have a different sharing method for each country or region.} 

%% What's wrong with traditional method - Why we need a new cost allocation methodology??
\textcolor{red}{However, classic methods cause irrationality. In the case of postage-stamp, since the ratio is determined by injection and demand without considering the specific contribution of each utility to the expansion line, the derived results cannot be determined to be the result of pure expansion line~\cite{shahidehpour2003market}. Furthermore, the network complexity is increased by the inflow of various resources into the system, and the accuracy of the existing method is further reduced as relay facilities for system stability are linked together with the line expansion.}

%% Cost allocation methodology reviews~~~~~~~~~~~~~~~
%% Usage method
\textcolor{red}{Much literature has been studied to solve the classic method. One of that is calculating the line usage mathematically line for each utility mathematically. In~\cite{conejo2007z_, abhyankar2006optimization}, AC power flow is calculated using the impedance of each line to grasp the current flow adjacent to the line requiring cost allocation. After that, usage of demand and generator using flow is defined, and costs are allocated through annual average flow. In~\cite{galiana2003transmission, gil2005multiarea}, using equivalent bilateral exchange (EBE) method is described. EBE uses impedance to calculate the sensitivity matrix between demand and generation. In other words, nodes and nodes connected to physical lines by Kirchhoff's law are equivalent to power, and the sensitivity between nodes can be realized by a matrix. By calculating this in a chain, the sensitivity of generation or demand to the line can be determined, and the usage of each node is calculated using DC power flow. But, the usage-based method cannot measure the contribution of relay facilities likewise classic methods.}

%% Game Theory
\textcolor{red}{Another cost allocation method is benefit-based game theory. Since the cost allocation through game theory is mainly based on the benefits of each utility according to the line expansion, it can derive results without contribution.Usually, when using game theory to allocate costs, problem is solved by using one of various solutions such as shapely value, kernel, or nucleolus based on benefit. In~\cite{tsukamoto1996allocation}, typically using cooperative game theory (CGT) based on Pareto efficiency is used to solve the allocating problem. The paper~\cite{ruiz2007effective} used Shapely value and~\cite{evans2003cost} used kernel theory. In~\cite{zolezzi2002transmission, erli2005transmission} shows nucleolus.}

%% Another problem
\textcolor{red}{However, as the system became GM, the aforementioned method faced other problems. For example, in a kind of RES, PV operates at maximum in peak hours~\cite{edwards2009energy}, and in this case, the classical cost allocation method may result in lower overall time to time line utilization but relatively higher cost allocating compared to base generators. Of course, in the case of resources that cannot control generation volume, the degree of line utilization is corrected by multiplying the factor called contribution~\cite{chung2005transmission}, but it is not accurate considering only fragmented parts. In addition, connecting RES with energy storage service (ESS) may affect cost allocating by intentionally storing energy during hours and limiting bidding capacity. Accordingly, the network itself becomes complicated, and a new cost allocation system is required when various forms of resources appear in distribution.}

%% New discussable content
\textcolor{red}{In a situation where renewable energy is prevalent, the cost allocation of expanding the distribution needs to be discussed anew. First, uncertain factors such as energy production and demand for RES should be dealt with stochastically. Existing cost allocation tends to depend on the resulting value without considering the reliability of the predicted value. Cost allocation through fragmented estimates cannot guarantee fair cost allocation in the highly volatile GM market. Second, in allocation, it is necessary to set a specific period, not a peak. Ensuring net stability in peak periods is the dominant objective of the line expansion, but as mentioned above, costs may be biased toward specific RESs.}

\textcolor{red}{In this paper, we propose a robust distribution network expansion cost allocation method in GM situation. First, we proceed probabilistic long-term forecast for demand and RES's generation.}
