...
%at every hour on multiple nodes

\begin{abstract}
\textcolor{blue}{We allocate transmission expansion costs among stakeholders in justifiable way based on the long-term electricity load forecasting 

 After few months, we cannot forecast the demand with the same accuracy of short-term forecasting, because we cannot obtain the hourly numerical weather prediction (NWP), so we forecast the probabilistic distribution of load. We sample future weather scenarios through the Gibbs sampling from multivariate distributions of temperature and dew point from eight weather stations for several years. Then, we convert the weather scenarios into the load distributions, which are combined to build the final distribution through the advanced kernel density estimation. Finally, we forecast the total load by combining the load from several states in a stepwise manner by using the hierarchical graphical model. 
%
We use the predicted load scenarios to allocate transmission expansion costs for future increased expected load. 
%
The total cost can be calculated based on the power transfer distribution factors, but the transmission costs should be allocated by the cooperative competition among stakeholders.
%
We verify our transmission cost allocation algorithm based on long-term  load forecasting by using the data from the Global Energy Forecasting Competition 2017. For forecasting performance, our approach outperforms the benchmarks at all zones. Furthermore, allocated costs depend on profits obtained from the transmission expansion, so our allocation algorithm is justifiable.}

%  to allocate transmission expansion costs  based on predicted power flow on a transmission network.
