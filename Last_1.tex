...
\begin{abstract}
\textcolor{blue}{We forecast the long-term electricity load at every hour on multiple nodes to allocate transmission expansion costs among stakeholders in an efficient way based on predicted power flow on a transmission network. After few months, we cannot obtain the hourly numerical weather prediction (NWP), and we cannot forecast the demand with the same accuracy of short-term forecasting, so we forecast the probabilistic distribution of load.


 
 We sample weather scenarios after a month through the Gibbs sampling from multivariate distributions of temperature and dew point observed from eight weather stations for several years. Then, we convert the weather scenarios into the load distributions, which are combined to build the final distribution through the advanced kernel density estimation.}
