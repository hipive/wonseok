...
\textcolor{red}{Typical SCG is defined by the following formula. 
\[\Gamma  = (N,{\nu _S},{\kappa _i})\] \[{\nu _S} = [{\nu _S} \in {L^1}(\Re )|{\nu _S} = \int {\rho (S) \cdot \theta (S)dt} ]\] Where $N$ means a set of players participating in game, and $S (S \in N)$ means a subset of players called coalition. The first element of the SCG's tuple is player, and the second element is random variable of pay-off dependent on the coalition. A random variable $\nu_S$ is in a space that satisfied ${\nu _S} \to {L^1}(\Re )$, and consequently contains the probability of a particular coalition ($\rho(S)$) and premium ($\theta(S)$)for it. The third element, $\kappa_i$ indicates reasonable behavior of the player. That is, cooperative games assume that players set their premium in advantageous direction, such as profit maximization or risk avoidance.}

\textcolor{red}{There are two main operating principles of SCG, one is that pay-off is determined by probability using the fact that future observations are not known, and the other is that premium based on probability can be transferred to other players.}
\subsection{Core}
% Why use core
