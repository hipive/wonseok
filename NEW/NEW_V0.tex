...
\begin{document}
\title{Distribution Network Expansion Cost Allocation Using Stochastic Cooperative Game Theory}
...
\begin{abstract}
\textcolor{red}{In this paper, we propose a cost allocating methodology for net work expansion in distribution where renewable energy has penetrated. Common network expansion cost, including general distribution, are allocated by mathematically calculating contribution of the line. In case of distribution, expected demand of each consumption zone was determined and contribution calculated. However, the higher penetration ratio of renewable energy in distribution, the higher volatility of demand. Therefore, the cost allocating of existing methods does not accept this volatility, so it could cause irrationality if applied to current network. In addition, the postage-stamp method adopted by most countries is calculated only through amount of flow during peak, so it is not fair to allocate costs without considering overall benefits of network expansion.Therefore, we intend to solve the cost allocation problem by using stochastic cooperative game theory (SCGT) to consider above problems. To apply SCGT, we first make a demand map according to confidence interval through quantile regression using Gibbs sampling and XGBoost. Quantile regression allows us to calculate the utility benefits that take charge of demand region by outputting discrete probabilistic demand distribution over time. Such benefit calculation result in discrete representation of stochastic benefit associated with time for each period, and by substituting this for SCGT, we can get the cos t allocating results by time. To produce weighted mean according to peak of previously obtained cost allocating result, we use fuzzy weighted average (FWA) theory, and we can finally get cost allocation ratio that each utility should pay for network expansion}
\end{abstract}
